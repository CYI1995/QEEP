...
for sth in range(N_T_total):
    ...
    Total_length = 120*(2**(sth))
    Sample_number = int(4*math.log(Total_length))
    ...
    for count in range(Batch):
        ...
        err = min(abs(2*math.pi*(output) - gs_energy - 2*math.pi),abs(2*math.pi*(output) - gs_energy))
...
np.save('Variance_err_r3.npy',Variance)
np.save('Average_err_r3.npy',Average_err)
np.save('Average_Total_time_r3.npy',Total_time_set)
np.save('Average_Maximal_time.npy',Maximal_time_set)